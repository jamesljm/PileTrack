...
export function MicropilingForm({ control, siteId }: MicropilingFormProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <CollapsibleSection title="Details" defaultOpen>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={control} name="pileId" render={({ field }) => (
            <FormItem><FormLabel>Pile ID</FormLabel><FormControl><Input placeholder="MP-001" className="min-h-[44px]" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={control} name="diameter" render={({ field }) => (
            <FormItem><FormLabel>Diameter (mm)</FormLabel><FormControl><Input type="number" className="min-h-[44px]" {...field} onChange={e => field.onChange(Number(e.target.value))} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <FormField control={control} name="depth" render={({ field }) => (
            <FormItem><FormLabel>Total Depth (m)</FormLabel><FormControl><Input type="number" step="0.1" className="min-h-[44px]" {...field} onChange={e => field.onChange(Number(e.target.value))} /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={control} name="drillingMethod" render={({ field }) => (
            <FormItem><FormLabel>Drilling Method</FormLabel><FormControl><Input placeholder="Rotary, Percussive..." className="min-h-[44px]" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        ...
        <div className="mt-4">
          <FormField control={control} name="groutMixRatio" render={({ field }) => (
            <FormItem><FormLabel>Grout Mix Ratio</FormLabel><FormControl><Input placeholder="e.g. 0.45 w/c" className="min-h-[44px]" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <div className="mt-4">
          <FormField control={control} name="reinforcementType" render={({ field }) => (
            <FormItem><FormLabel>Reinforcement Type</FormLabel><FormControl><Input placeholder="Steel bar, Self-drilling..." className="min-h-[44px]" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        ...
        <div className="mt-4">
          <FormField control={control} name="flushType" render={({ field }) => (
            <FormItem><FormLabel>Flush Type</FormLabel>
              <Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger className="min-h-[44px]"><SelectValue placeholder="Select flush type" /></SelectTrigger></FormControl>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <FormField control={control} name="inclination" render={({ field }) => (
            <FormItem><FormLabel>Inclination (degrees) - Optional</FormLabel><FormControl><Input type="number" step="0.1" className="min-h-[44px]" {...field} onChange={e => field.onChange(e.target.value ? Number(e.target.value) : undefined)} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <div className="mt-4">
          <FormField control={control} name="casingLength" render={({ field }) => (
            <FormItem><FormLabel>Casing Length (m) - Optional</FormLabel><FormControl><Input type="number" step="0.1" className="min-h-[44px]" {...field} onChange={e => field.onChange(e.target.value ? Number(e.target.value) : undefined)} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
      </CollapsibleSection>
      ...
      <CollapsibleSection title="Timing">
        {(["setup", "drilling", "grouting", "reinforcement"] as const).map((stage) => (
